...
fn run_client_app(config: LauncherConfig) {
    match config.example {
        Example::SimpleBox => {
            simple_box::run_client(
                config.client_id,
                config.server_addr(),
                match config.client_transport {
                    ClientTransport::Udp => simple_box::TransportType::Udp,
                    ClientTransport::WebTransport => simple_box::TransportType::WebTransport,
                    ClientTransport::WebSocket => simple_box::TransportType::WebSocket,
                },
            );
        }
    }
