//! Client module - connects to server and receives delta-compressed player positions.
...
impl Plugin for ExampleClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(Update, log_players);
    }
}
...
fn on_connected(trigger: On<Add, Connected>, names: Query<&Name>) {
    if let Ok(name) = names.get(trigger.entity) {
        ...
        info!("📦 Receiving delta-compressed position updates");
    }
}
