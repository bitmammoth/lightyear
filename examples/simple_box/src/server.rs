...
impl Plugin for ExampleServerPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.init_resource::<ServerTransportConfig>();
        // NOTE: Startup system is NOT added here - it's added by run_server() or run_host_client() in main.rs
        // This allows proper ordering in host-client mode where server must start before client
...
/// Configuration for which transports to enable on the server
#[derive(Resource)]
...
pub fn startup(mut commands: Commands, config: Res<ServerTransportConfig>) -> Result {
    ...
    let enabled: Vec<&str> = [
        config.enable_udp.then_some("UDP"),
        config.enable_webtransport.then_some("WebTransport"),
        config.enable_websocket.then_some("WebSocket"),
    ].into_iter().flatten().collect();
    info!("\n✅ Server initialized with transports: {}. All clients connect to Server {:?}\n", enabled.join(", "), server);
    Ok(())
}
