...
impl Plugin for ClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_observer(on_player_color_added);
...
fn handle_predicted_spawn(
    ...
    if let Ok(mut color) = predicted.get_mut(entity) {
        ...
        info!("🎯 Predicted player spawned: {:?} - Adding InputMarker + ActionState!", entity);
        ...
        info!("🆔 PlayerId added to {:?} but not Predicted (probably interpolated)", entity);
    }
}
...
fn draw_players(
    ...
    // Log every second when we have players
    if count > 0 && now - *last_log > 1.0 {
        *last_log = now;
        info!("🎨 draw_players: Drawing {} players", count);
    }
    
    // Always draw a reference point at origin so we know gizmos work
    gizmos.circle_2d(Isometry2d::IDENTITY, 20.0, Color::WHITE);
    
    for (_entity, position, color) in &players {
        // Draw the player box
        gizmos.rect_2d(
        ...
        // Also draw a cross at player position for visibility
        gizmos.line_2d(
            position.0 - Vec2::new(30.0, 0.0),
            position.0 + Vec2::new(30.0, 0.0),
            color.0,
        );
        gizmos.line_2d(
            position.0 - Vec2::new(0.0, 30.0),
            position.0 + Vec2::new(0.0, 30.0),
            color.0,
        );
    }
...
fn log_players(
    players: Query<(Entity, &PlayerPosition, Option<&Predicted>, Option<&Interpolated>), With<PlayerId>>,
    time: Res<Time>,
    mut last_log: Local<f32>,
) {
    let now = time.elapsed_secs();
...
/// Log when PlayerColor is added
fn on_player_color_added(trigger: On<Add, PlayerColor>, query: Query<&PlayerColor>) {
    if let Ok(color) = query.get(trigger.entity) {
        info!("🎨 PlayerColor added to {:?}: {:?}", trigger.entity, color.0);
    }
}
