//! Client module - connects to server and receives replicated shapes.
...
impl Plugin for ExampleClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        // Predicted spawn handler
        app.add_observer(handle_predicted_spawn);
        app.add_observer(handle_interpolated_spawn);
        app.add_systems(Update, log_replicated_shapes);
    }
}
...
/// Log count of replicated shapes periodically
fn log_replicated_shapes(
    ...
    if count != *last_count || timer.just_finished() {
        if count > 0 {
            info!("📐 Replicated shapes: {}", count);
        }
        *last_count = count;
    }
}
...
/// Apply movement to predicted entities
fn player_movement(
...
/// When predicted player spawns, add InputMarker
fn handle_predicted_spawn(
    ...
    if let Ok(mut color) = predicted.get_mut(entity) {
        ...
        info!("🎮 Predicted player spawned: {:?}, adding InputMarker", entity);
        commands.entity(entity).insert(InputMarker::<Inputs>::default());
    }
}

/// When interpolated player spawns, adjust color
