//! Multi-Transport Deterministic Replication Example
//!
//! Demonstrates deterministic replication with checksum verification across
//! UDP, WebTransport, and WebSocket transports.
//! All clients simulate all physics locally, with input-based rollback only.
//!
//! Run with:
//! - `cargo run -- server`
...
#[derive(Parser)]
#[command(name = "multi_transport_deterministic_replication")]
#[command(about = "Multi-transport deterministic replication demo")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
