...
### Completed Examples ✅
1. `multi_transport_simple_box` - Basic replication
2. `multi_transport_distributed_authority` - Authority transfer
3. `multi_transport_network_visibility` - Interest management
4. `multi_transport_client_replication` - Client->server replication
5. `multi_transport_replication_groups` - Linked entities
6. `multi_transport_priority` - Priority-based replication
7. `multi_transport_delta_compression` - Delta compression
8. `multi_transport_auth` - ConnectToken authentication
9. `multi_transport_bevy_enhanced_inputs` - BEI input integration
10. `multi_transport_avian_physics` - 2D physics with avian2d
11. `multi_transport_avian_3d_character` - 3D character controller
12. `multi_transport_deterministic_replication` - Deterministic with checksums
